...
client.on('voiceStateUpdate', (oldMember, newMember) => {
 ...
 // if (!logA[oldMember.guild.id]) return;
  ...
  let oldUserChannel = oldMember.voiceChannel
    ...
    if(newUserChannel === newMember.name){

  
    const embed = new Discord.MessageEmbed()
    .setColor("BLUE")
    .setDescription(`<@${newMember.id}> adlı kullanıcı \` #${newUserChannel} \`  isimli sesli kanala giriş yaptı!`)
    kanal.send(embed);
    

    const embed2 = new Discord.MessageEmbed()
    .setColor("RED")
    .setDescription(`<@${newMember.id}>  adlı kullanıcı sesli kanaldan çıkış yaptı!`)
    kanal.send(embed2);
